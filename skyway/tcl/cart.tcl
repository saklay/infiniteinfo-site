...
proc CartUpdate { } {
    ...
    #
    # Check out the action field to determine what to do.
    #
    set action [ns_set get $formSet action]

    switch $action {
	"Continue Shopping" {
	    set refererurl [ns_set get $formSet refererurl]
	    if {![string match "" $refererurl]} {
		ns_returnredirect $refererurl
		return 0
	    }
	}
	"Check Out" {
	    set orderurl [ns_set get $formSet orderurl]
	    if {![string match "" $orderurl]} {
		ns_returnredirect $orderurl
		return 0
	    }
	}
    }
	...
	if {[regexp {^[0-9]+$} $key] && [regexp {^[1-9][0-9]*$} $value] } {
		ns_set update $cartSet $key $value
	} else {
	    if {[string match "remove" $key]} {
